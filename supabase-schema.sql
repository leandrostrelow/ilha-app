...
begin
  ...
  if selected_table_id is not null then
    ...
    if not found then
      begin
        insert into public.bar_orders (
          table_id, public_access_id, customer_name, source, notes
        ) values (
          selected_table_id, access_card_id, customer_value, access_source,
    ...
    insert into public.bar_orders (
      public_access_id, customer_name, source, notes
    ) values (
      access_card_id, customer_value, access_source,
      nullif(left(trim(coalesce(p_notes, '')), 300), '')
    ) returning * into order_row;
  end if;
  ...
  loop
    ...
    insert into public.bar_order_items (
      ...
      customer_name, source, notes
    ) values (
      order_row.id, product_row.id, product_row.name, quantity_value,
      product_row.sale_price, product_row.cost_price, customer_value,
...
create or replace function public.bar_public_order_status(p_tracking_token text)
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function public.bar_public_submit_order(text, text, uuid, jsonb, text) from public;
revoke all on function public.bar_public_order_status(text) from public;
grant execute on function public.bar_public_menu(text) to anon, authenticated;
grant execute on function public.bar_public_submit_order(text, text, uuid, jsonb, text) to anon, authenticated;
grant execute on function public.bar_public_order_status(text) to anon, authenticated;
