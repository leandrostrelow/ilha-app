...
const baseMigrationName = '20260914155856_tournament_group_stage.sql';
const groupMigrationName = '20260914164758_generalize_tournament_group_stage.sql';
const [adminApi, baseMigration, migration] = await Promise.all([
  readFile(path.join(projectRoot, 'supabase/functions/tournament-admin-api/index.ts'), 'utf8'),
  readFile(path.join(projectRoot, 'supabase/migrations', baseMigrationName), 'utf8'),
  readFile(path.join(projectRoot, 'supabase/migrations', groupMigrationName), 'utf8'),
]);

test('API autoriza generateGroup com três ou mais inscrições confirmadas distintas', () => {
  ...
  assert.match(adminApi, /new Set\(athleteIds\)\.size !== registrations\.length/);
  assert.match(adminApi, /registrations\.length \* \(registrations\.length - 1\)/);
  assert.match(adminApi, /client\.rpc\("tournament_replace_group_stage_atomic"/);
});

test('RPC cria todas as combinações do grupo e uma final privada', () => {
  ...
  assert.match(migration, /expected_group_match_count := \(athlete_count \* \(athlete_count - 1\)\) \/ 2/);
  assert.match(migration, /for first_index in 1\.\.\(athlete_count - 1\) loop/);
  assert.match(migration, /for second_index in \(first_index \+ 1\)\.\.athlete_count loop/);
  assert.match(migration, /p_athlete_ids\[first_index\]/);
  assert.match(migration, /p_athlete_ids\[second_index\]/);
  assert.match(migration, /'PENDING',[\s\S]*group_match_no \* 1000 \+ 10,[\s\S]*false/);
  assert.match(migration, /expected_group_match_count \+ 1,[\s\S]*'FINAL',[\s\S]*false/);
  assert.doesNotMatch(
    migration.slice(
...
test('geração valida novamente todos os inscritos e substitui os jogos atomicamente', () => {
  const rpc = migration.slice(
    migration.indexOf('create or replace function public.tournament_replace_group_stage_atomic'),
  );
  assert.match(rpc, /public\.has_tournament_permission\('tournaments\.write'\)/);
  assert.match(rpc, /athlete_count < 3/);
  assert.match(rpc, /confirmed_count <> athlete_count/);
  assert.match(rpc, /pending_count <> 0/);
  assert.match(rpc, /requested_athlete_count <> athlete_count/);
  assert.match(rpc, /matched_confirmed_count <> athlete_count/);
  ...
  assert.match(rpc, /jsonb_array_length\(inserted_matches\) <> expected_group_match_count \+ 1/);
  assert.match(migration, /draw_size in \(2, 4, 8, 16, 32, 64, 128\)/);
  assert.match(migration, /draw_format = 'GROUPS_AND_KNOCKOUT'[\s\S]*draw_size >= 3/);
  assert.match(migration, /tournament_categories_group_capacity_check/);
  assert.match(migration, /max_entries is not distinct from draw_size/);
});
...
test('geração mata-mata restaura o formato da categoria na mesma transação', () => {
  ...
  const wrapper = baseMigration.slice(
    baseMigration.indexOf('create or replace function public.tournament_replace_single_elimination_atomic'),
    baseMigration.indexOf('create or replace function public.tournament_replace_group_stage_atomic'),
  );
...
test('classificação oficial aplica os critérios e só preenche a final com desempate resolvido', () => {
  ...
  assert.match(refresh, /expected_group_match_count := case/);
  assert.match(refresh, /player_count \* \(player_count - 1\)/);
  assert.match(refresh, /completed_match_count = expected_group_match_count/);
  assert.match(refresh, /win_tie_count = 2/);
  assert.match(refresh, /head_to_head_won/);
  assert.match(refresh, /win_tie_count >= 3/);
...
test('super tie conta como set, não como games, e placar compacto usa apenas o game principal', () => {
  const metrics = baseMigration.slice(
    baseMigration.indexOf('create or replace function private.tournament_group_score_metrics'),
    baseMigration.indexOf('create or replace function private.tournament_group_stage_standings'),
  );
...
test('grupo fechado serializa inscrições, exige zero pendências e fixa a lotação no total atual', () => {
  ...
  assert.match(groupRpc, /max_entries = athlete_count/);
  assert.match(groupRpc, /draw_size = athlete_count/);
...
test('troca de participantes da final é bloqueada no banco quando já existem palpites', () => {
  const guard = baseMigration.slice(
    baseMigration.indexOf('create or replace function private.guard_tournament_group_final_predictions'),
    baseMigration.indexOf('create or replace function public.tournament_replace_single_elimination_atomic'),
  );
