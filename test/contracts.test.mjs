...
test('Ilha Open oferece a Espacial correta por mais R$ 80 no mesmo pagamento', () => {
  ...
  assert.match(functionSource(tournamentSource, 'statusLabel'), /isRegistrationExplicitlyClosed\(row\)[\s\S]*Inscrições fechadas/);
  assert.match(functionSource(tournamentSource, 'statusClass'), /isRegistrationExplicitlyClosed\(row\)/);
...
test('Ilha Open possui PWA próprio, atalhos, Push protegido e somente patrocinadores configurados', async () => {
  ...
  assert.match(tournamentSource, /--safe-top: env\(safe-area-inset-top, 0px\)/);
  assert.match(tournamentSource, /padding-top: var\(--safe-top\)/);
  assert.match(tournamentSource, /top: calc\(66px \+ var\(--safe-top\)\)/);
  assert.match(tournamentSource, /100dvh - 20px - var\(--safe-top\) - var\(--safe-bottom\)/);
  assert.match(tournamentSource, /<div class="brand-link"><img/);
  assert.doesNotMatch(tournamentSource, /<a class="brand-link"/);
  ...
  assert.match(tournamentSource, /id="tournamentPwaOnboarding"/);
  assert.match(tournamentSource, /Logo do Ilha Open 2026/);
  assert.match(tournamentSource, /TOURNAMENT_INSTALL_PROMPT_SESSION_KEY/);
  assert.match(tournamentSource, /TOURNAMENT_PUSH_PROMPT_SESSION_KEY/);
  assert.match(tournamentSource, /TOURNAMENT_PUSH_PREFERENCE_KEY/);
  assert.match(functionSource(tournamentSource, 'loadPage'), /maybeShowTournamentPwaOnboarding\(\)/);
  const onboardingSource = functionSource(tournamentSource, 'maybeShowTournamentPwaOnboarding');
  assert.match(onboardingSource, /!tournamentAppInstalled\(\)/);
  assert.match(onboardingSource, /TOURNAMENT_INSTALL_PROMPT_SESSION_KEY/);
  assert.match(onboardingSource, /state\.tournamentNotificationsEnabled/);
  assert.match(onboardingSource, /tournamentPushPreference\(\) === 'disabled'/);
  assert.match(onboardingSource, /openTournamentPwaOnboarding\('notifications'\)/);
  const enableNotificationsSource = functionSource(tournamentSource, 'enableTournamentNotifications');
  assert.match(enableNotificationsSource, /setTournamentPushPreference\('enabled'\)/);
  const disableNotificationsSource = functionSource(tournamentSource, 'disableTournamentNotifications');
  assert.match(disableNotificationsSource, /action:'unsubscribe'/);
  assert.match(disableNotificationsSource, /subscription\.unsubscribe\(\)/);
  assert.match(disableNotificationsSource, /setTournamentPushPreference\('disabled'\)/);
  assert.match(functionSource(tournamentSource, 'updateTournamentPwaUi'), /Desativar notificações/);
  assert.match(functionSource(tournamentSource, 'handleTournamentPwaOnboardingKeydown'), /event\.key === 'Escape'/);
  assert.match(functionSource(tournamentSource, 'setTournamentOnboardingBackgroundInert'), /setAttribute\('inert',''\)/);
  ...
  assert.match(tournamentPublicPushSource, /action === "unsubscribe"/);
...
test('membro familiar ativo vira aluno operacional e pode receber plano e aula no ADM', () => {
  ...
  assert.match(serviceWorkerSource, /ilha-play-v242-agenda-match-editor/);
});
