...
-- The first staging execution exposed a PL/pgSQL ambiguity between the local
-- actor_id variable and the audit column with the same name. Rename the local
-- variable without rebuilding or exposing the private snapshot table. This is
-- intentionally idempotent: fresh environments already receive the corrected
-- function from the preceding migration.
do $migration$
declare
...
begin
  ...
  if function_definition like '%caller_id uuid :=%'
    ...
    -- Already corrected by the canonical migration.
    null;
  elsif function_definition like '%actor_id uuid :=%' then
    ...
    if corrected_definition not like '%caller_id uuid :=%'
       or corrected_definition not like '%restored_by = caller_id,%' then
      raise exception 'A correção da função de restauração não foi validada.'
        using errcode = '55000';
    end if;
...
-- SECURITY DEFINER wrappers still validate an authenticated administrator.
-- Remove direct service-role execution as an additional fail-closed layer.
